...
////
	...
	// 主机名映射
	$hostMap = array(
	    'otdb_server_a' => array('125.254.156.49',3308),
	    'otdb_server_b' => array('125.254.156.50', 3306),
	    'otdb_server_c' => array('125.254.156.39', 3305)
	);
	include './query_apply/ip_binds.php';
	$ip = '127.0.0.1';
	if (!isset($ip_bind[$ip])) {
	    exit('未获得访问授权！');
	}
	$userInfo = $ip_bind[$ip];
	if ($_POST['login'] !== $userInfo['identify']) {
	    exit('用户名或密码错误');
	}
	
	$_POST['host'] = $hostMap[$_POST["host"]][0];
	$_POST['port'] = $hostMap[$_POST["host"]][1];
	$_POST['login'] = 'p2pnew_bj_check';
	$_POST['password'] = 'p2pnew_check&*TTS';
